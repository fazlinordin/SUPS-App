...
# --- 1. SETTING AWAL ---
st.set_page_config(page_title="SUPS HJEM V7.4 (Updated)", layout="wide")
...
if menu == "📝 INPUT":
    ...
    with st.container(border=True):
        ...
        # Auto Huruf Besar untuk Nama
        nama_in = c1.text_input("Nama Pesakit:", key="nama_field").upper().strip()
        ic_in = c2.text_input("No. IC:", key="ic_field").strip()
    ...
    if st.session_state.bakul:
        ...
        # BUTANG SIMPAN (Di bawah sekali)
        if st.button("💾 SIMPAN SEMUA DATA & RESET", type="primary", use_container_width=True):
            if nama_in and ic_in and st.session_state.bakul:
                with st.spinner("⏳ Sedang menyimpan..."):
                    ...
                    try:
                        ...
                        st.session_state.bakul = []
                        # Cara reset input tanpa guna session state manual yang trigger error
...
# --- 4. SUMMARY (DENGAN WARNA & IC) ---
    ...
    if not df.empty:
        ...
        if not df_f.empty:
            ...
            matrix = {}
            # Masukkan IC dalam matrix supaya keluar di Excel
